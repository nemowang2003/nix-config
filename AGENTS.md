...
- `packages/` is loaded by `flake/packages.nix` via `inputs.haumea.lib.load`, with `loader = loaders.callPackage` and `transformer = self.lib.haumea.force-shallow-transformer` (defined in `lib/haumea.nix`): a top-level `.nix` file becomes a package, and a directory must contain `default.nix` (which is exported; other files are loaded but not exported). The result is the flake's `packages` output; modules reach it through `self.packages.${pkgs.system}.<name>` rather than a nixpkgs overlay.
